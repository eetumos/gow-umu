...
RUN dnf install -y glibc-langpack-en util-linux       \
                   mangohud gamescope                 \
                   foot fzf icoextract ImageMagick jq \
                   egl-utils vulkan-tools jstest-gtk

RUN <<-EOF
	dnf install -y https://github.com/Open-Wine-Components/umu-launcher/releases/latest/download/umu-launcher-1.2.6.fc41.rpm
